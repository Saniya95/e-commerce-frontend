  ...
  return (
    <div className="card group relative overflow-hidden transition-all duration-500 hover:translate-y-[-8px] bg-[#D183A9]">
      ...
      <div className="absolute top-3 right-3 z-20">
        <button className="bg-[#8D7DFA] hover:bg-[#7C6CE8] text-white rounded-full w-8 h-8 flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-lg">
          <span className="text-lg font-bold">+</span>
        </button>
      </div>

      {/* Category badge */}
      <div className="absolute top-3 right-14 z-10 bg-[#5D4E8C] bg-opacity-70 backdrop-blur-sm text-white text-xs px-2 py-1 rounded-full">
        {typeof category === "object" ? category?.name || "General" : category}
      </div>
      ...
      <div className="p-5 relative">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            ...
            {discount > 0 && (
              <span className="text-sm text-[#C9BBF7] line-through opacity-70">
                {formatPrice(price)}
              </span>
            )}
          </div>
