    ...
    try {
      ...
      try {
        ...
        if (paymentMethod === "cod") {
          // For COD, create a COD payment record
          const paymentResponse = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/api/payment/create/${orderData.order.id}`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          if (!paymentResponse.ok) {
            toast.error(paymentData.message || "Failed to create COD order");
            return;
          }

          // For COD, directly redirect to success page
          router.push(
            `/order-success?orderId=${orderData.order.id}&method=cod`
          );
          ...
          if (typeof window !== "undefined" && window.Razorpay) {
            const options = {
              ...
              handler: function (response) {
                ...
                router.push(
                  `/order-success?orderId=${orderData.order.id}&paymentId=${response.razorpay_payment_id}`
                );
              ...
              prefill: {
                name: user?.fullName || "",
                email: user?.email || "",
                contact: addresses[selectedAddress]?.mobile || "",
              },
              theme: {
                color: "#ff6b9d",
              },
            ...
            router.push(`/payment?orderId=${orderData.order.id}`);
          }
        }
      } catch (parseError) {
                    ...
                    <div className="space-y-3">
                      <label className="flex items-center p-4 bg-white/5 rounded-xl border border-white/10 cursor-pointer hover:bg-white/10 transition-all">
                        <input
                          ...
                          className="mr-3 text-[#ff6b9d] focus:ring-[#ff6b9d]"
                        />
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-gray-400 text-sm">
                              Pay securely with card/UPI/wallet
                            </p>
                          </div>
                        </div>
                      </label>
